...
.metric-value { font-family: 'Space Mono', monospace; font-size: 2em; font-weight: 700; color: #64ffda; margin: 0; }
.metric-label { font-size: 0.78em; color: #8892b0; text-transform: uppercase; letter-spacing: 1.5px; margin-top: 4px; }
.section-header {
...
with st.sidebar:
    ...
    """, unsafe_allow_html=True)
    page = st.radio("Navigate", ["Project Overview","Data Exploration","Model Analysis","Live Prediction"], label_visibility="collapsed")
    st.markdown("""
    <hr style='border-color:#1e3a5f; margin:20px 0;'>
    <div style='font-size:0.75em; color:#8892b0; padding:0 4px;'>
        <b style='color:#64ffda;'>Dataset</b><br>Store Sales — Time Series<br>Corporación Favorita, Ecuador<br><br>
...
if page == "Project Overview":
    ...
    st.markdown("<p style='color:#8892b0; font-size:1em;'>Time Series Forecasting · Corporación Favorita · Ecuador</p>", unsafe_allow_html=True)
    st.markdown("---")

    for col, (val, label) in zip(st.columns(4), [
        ("3,000,888","Training Rows"),("54","Stores"),("33","Product Families"),("LightGBM","Algorithm")]):
        with col:
    ...
    st.markdown("<div class='insight-box'>Businesses face persistent difficulty predicting future sales accurately due to fluctuating customer demand, seasonal cycles, promotions, and macroeconomic signals such as oil prices in Ecuador's economy. Inaccurate forecasts lead to overstock, stockouts, and lost revenue. This system replaces traditional methods with a Machine Learning pipeline that learns complex patterns directly from 4 years of historical data across 54 stores and 33 product families.</div>", unsafe_allow_html=True)

    st.markdown("<div class='section-header'>Technical Pipeline</div>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    left_steps  = [("01","Data Collection","6 CSV files — train, test, stores, oil, holidays, transactions"),
    ...
    right_steps = [("05","Model Training","LightGBM — 1000 trees with early stopping on 30-day holdout"),
    ...
    for col, steps in [(col1, left_steps),(col2, right_steps)]:
        with col:
            for num, title, desc in steps:
                st.markdown(f"""<div style='display:flex;gap:16px;margin-bottom:16px;align-items:flex-start;'>
                    <div style='font-family:Space Mono;font-size:1.4em;color:#64ffda;opacity:0.4;min-width:32px;'>{num}</div>
                    <div><div style='color:#ccd6f6;font-weight:500;margin-bottom:2px;'>{title}</div>
                    <div style='color:#8892b0;font-size:0.85em;'>{desc}</div></div></div>""", unsafe_allow_html=True)
    ...
    fig = go.Figure(data=[go.Table(
        header=dict(values=list(comp.columns), fill_color=C["panel"],
                    font=dict(color=C["teal"], family="Space Mono", size=11),
                    align="left", line_color=C["border"], height=36),
        cells=dict(values=[comp[c] for c in comp.columns], fill_color=C["navy"],
                   font=dict(color=[C["text"],C["text"],C["text"],C["text"],
                                    [C["muted"],C["muted"],C["muted"],C["teal"],C["muted"]]],
                             size=12),
...
# PAGE 2 — DATA EXPLORATION
    ...
    family_sales = pd.DataFrame({
        "family":["GROCERY I","BEVERAGES","PRODUCE","CLEANING","DAIRY","BREAD/BAKERY","POULTRY","MEATS","PERSONAL CARE","DELI"],
    ...
    if train is not None:
        ...
        monthly_avg2 = train.groupby("month")["sales"].mean().reset_index()
        monthly_avg2["month_name"] = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][:len(monthly_avg2)]
        monthly_avg = monthly_avg2.rename(columns={"sales":"sales"})
        train["day_of_week"] = train["date"].dt.dayofweek
        dow2 = train.groupby("day_of_week")["sales"].mean().reset_index()
        dow2["day_name"] = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"]
        dow_avg = dow2.rename(columns={"sales":"sales"})
        ...
        o2 = oil.copy(); o2["dcoilwtico"] = o2["dcoilwtico"].ffill()
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=daily_sales["date"], y=daily_sales["total_sales"],
        mode="lines", line=dict(color=C["teal"],width=1.2),
        fill="tozeroy", fillcolor="rgba(100,255,218,0.07)", name="Daily Sales"))
    fig.update_layout(**PLOTLY_LAYOUT, height=300,
        title="Total Sales Across All 54 Stores and 33 Families", yaxis_tickformat=".2s")
    st.plotly_chart(fig, use_container_width=True)
    ...
    col1, col2 = st.columns(2)
    with col1:
        fig = go.Figure(go.Bar(x=monthly_avg["month_name"], y=monthly_avg["sales"],
            marker=dict(color=monthly_avg["sales"],
    ...
    with col2:
        fig = go.Figure(go.Bar(x=dow_avg["day_name"], y=dow_avg["sales"],
            marker=dict(color=[C["teal"] if i<5 else C["red"] for i in range(7)],
    ...
    fig = go.Figure(go.Bar(x=family_sales["sales"], y=family_sales["family"], orientation="h",
        marker=dict(color=PALETTE[:len(family_sales)], line=dict(color=C["border"],width=0.5))))
    fig.update_layout(**PLOTLY_LAYOUT, height=340,
        title="Total Sales by Product Family (Billions)", xaxis_ticksuffix="B")
    st.plotly_chart(fig, use_container_width=True)
    ...
    with col1:
        ...
        fig = go.Figure(go.Bar(x=type_avg["type"], y=type_avg["sales"],
    ...
    with col2:
        ...
        fig.add_trace(go.Scatter(x=combined["date"], y=combined["total_sales"],
            name="Total Sales", line=dict(color=C["teal"],width=1)), secondary_y=False)
        fig.add_trace(go.Scatter(x=combined["date"], y=combined["dcoilwtico"],
            name="Oil Price", line=dict(color=C["red"],width=1,dash="dot")), secondary_y=True)
...
# PAGE 3 — MODEL ANALYSIS
    ...
    for col,(val,name,sub) in zip(st.columns(4),[
        ("0.4231","RMSLE","Competition Metric"),("312.84","MAE","Mean Abs. Error"),
        ("689.21","RMSE","Root Mean Sq. Error"),("Top 40%","Kaggle Rank","Benchmark")]):
        with col:
            st.markdown(f'<div class="metric-card"><div class="metric-value">{val}</div><div class="metric-label">{name}</div><div style="color:#8892b0;font-size:0.75em;margin-top:4px;">{sub}</div></div>', unsafe_allow_html=True)
    ...
    for (r,c),fam in zip([(1,1),(1,2),(2,1),(2,2)],families):
        ...
        show = (fam=="GROCERY I")
        fig.add_trace(go.Scatter(x=val_dates,y=actual,mode="lines",name="Actual" if show else None,
            line=dict(color=C["teal"],width=2),showlegend=show),row=r,col=c)
        fig.add_trace(go.Scatter(x=val_dates,y=predicted,mode="lines",name="Predicted" if show else None,
            line=dict(color=C["red"],width=2,dash="dot"),showlegend=show),row=r,col=c)
    ...
    fig = go.Figure(go.Histogram(x=errors,nbinsx=80,
    ...
    features   = ["sales_lag_365","sales_lag_16","sales_lag_30","sales_lag_60",
    ...
    fig = go.Figure(go.Bar(
        ...
        marker=dict(color=[C["teal"] if v>10 else C["blue"] if v>3 else C["muted"] for v in imp_df["importance"]],
                    line=dict(color=C["border"],width=0.5)),
    ...
    with col2: st.markdown("<div class='insight-box'><b style='color:#64ffda'>transactions</b> and recent lags capture short-term momentum — busy stores stay busy.</div>", unsafe_allow_html=True)
    with col3: st.markdown("<div class='insight-box'><b style='color:#00b4d8'>Calendar features</b> (month, week_of_year, day_of_week) together contribute ~12% capturing seasonality.</div>", unsafe_allow_html=True)
...
# PAGE 4 — LIVE PREDICTION
    ...
    FEATURE_COLS = ["store_nbr","family_encoded","store_type_encoded","cluster","city_encoded",
    ...
    FAMILY_MAP = {"AUTOMOTIVE":0,"BABY CARE":1,"BEAUTY":2,"BEVERAGES":3,"BOOKS":4,"BREAD/BAKERY":5,
                  "CELEBRATION":6,"CLEANING":7,"DAIRY":8,"DELI":9,"EGGS":10,"FROZEN FOODS":11,
                  "GROCERY I":12,"GROCERY II":13,"HARDWARE":14,"HOME AND KITCHEN I":15,
                  "HOME AND KITCHEN II":16,"HOME APPLIANCES":17,"HOME CARE":18,"LADIESWEAR":19,
                  "LAWN AND GARDEN":20,"LINGERIE":21,"LIQUOR,WINE,BEER":22,"MAGAZINES":23,
                  "MEATS":24,"PERSONAL CARE":25,"PET SUPPLIES":26,"PLAYERS AND ELECTRONICS":27,
                  "POULTRY":28,"PREPARED FOODS":29,"PRODUCE":30,"SCHOOL AND OFFICE SUPPLIES":31,"SEAFOOD":32}
    STORE_TYPE_MAP = {"A":0,"B":1,"C":2,"D":3,"E":4}
    CITY_MAP = {"Ambato":0,"Babahoyo":1,"Cayambe":2,"Cuenca":3,"Daule":4,"El Carmen":5,
                "Esmeraldas":6,"Guaranda":7,"Guayaquil":8,"Ibarra":9,"Latacunga":10,
                "Libertad":11,"Loja":12,"Machala":13,"Manta":14,"Playas":15,"Puyo":16,
                "Quevedo":17,"Quito":18,"Riobamba":19,"Salinas":20,"Santo Domingo":21}
    ...
    if st.button("Generate Sales Prediction"):
        if model is None:
            ...
            features = pd.DataFrame([{
                "store_nbr":store_nbr,"family_encoded":FAMILY_MAP[family],
                "store_type_encoded":STORE_TYPE_MAP[store_type],"cluster":cluster,
                "city_encoded":CITY_MAP[city],"year":d.year,"month":d.month,
                "day_of_month":d.day,"day_of_week":d.dayofweek,
                "week_of_year":d.isocalendar()[1],"is_weekend":int(d.dayofweek>=5),
                "dcoilwtico":oil_price,"is_holiday":1 if is_holiday=="Yes" else 0,
                "onpromotion":onpromotion,"has_promotion":int(onpromotion>0),
                "transactions":transactions,"sales_lag_16":lag_16,
                "sales_lag_30":lag_30,"sales_lag_60":lag_60,"sales_lag_365":lag_365,
            }])
            ...
            with col1:
                ...
                summary = {"Store":store_nbr,"Family":family,"Date":str(date_input),
                ...
                fig = go.Figure(data=[go.Table(
                    header=dict(values=["Field","Value"],fill_color=C["panel"],
                                font=dict(color=C["teal"],family="Space Mono",size=11),
                                align="left",line_color=C["border"],height=32),
                    cells=dict(values=[sdf["Field"],sdf["Value"]],fill_color=C["navy"],
                               font=dict(color=C["text"],size=12),align="left",
                               line_color=C["border"],height=30))])
                fig.update_layout(**PLOTLY_LAYOUT,height=300)
                st.plotly_chart(fig, use_container_width=True)
            with col2:
                ...
                fig = go.Figure(go.Bar(
                    ...
                    marker=dict(color=[C["teal"],C["blue"],C["purple"],C["yellow"]],
                                line=dict(color=C["border"],width=0.5))))
                fig.add_hline(y=prediction,line=dict(color=C["red"],dash="dash",width=1.5))
                fig.add_annotation(text=f"Prediction: {prediction:,.0f}",
